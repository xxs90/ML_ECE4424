...
def polynomial_kernel(row_data, col_data, order):
    ...
    #############################################
    k_kernel = (row_data.T.dot(col_data) + 1) ** order
...
def rbf_kernel(row_data, col_data, sigma):
    ...
    #############################################
    x_x = np.sum(row_data ** 2, 0, keepdims=True).T
    y_y = np.sum(col_data ** 2, 0, keepdims=True)
    x_y = row_data.T.dot(col_data)

    kernel_val = np.exp(-(x_x + y_y - 2 * x_y) / (2 * sigma ** 2))
