...
def mlp_objective(model, data, labels, loss_function):
    ...
    for i in reversed(range(num_layers - 1)):
        #######################################################################
        # TODO: insert your code to back-propagate errors from
        # layer (i + 1) to current layer i. Compute and store the delta value in layer_delta[i] 
        # (the gradient of loss w.r.t. activations at layer i). 
        # This computation should be about 1--2 lines of code.
        #######################################################################
    ...
    for i in range(num_layers):
        #######################################################################
        # TODO: insert your code to compute the gradient for the weights of layer i. 
        # Compute and store a value in layer_gradients[i], which will use
        # the layer_delta you previously computed as well as other
