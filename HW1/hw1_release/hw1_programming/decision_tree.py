...
def recursive_tree_train(data, labels, depth, max_depth, num_classes):
    ...
    node['max_gain'] = gain.argmax()
    split_feature = np.asarray(data[gain.argmax(), :]).ravel() > 0
    left_indices = split_feature.nonzero()[0]
    right_indices = (1 - split_feature).nonzero()[0]
...
def decision_tree_predict(data, model):
    ...
    if 'prediction' in model:
        ...
        # split data
        split_feature = data[model['max_gain'], :].ravel() > 0
        left_indices = split_feature.nonzero()[0]
        right_indices = (1 - split_feature).nonzero()[0]
        ...
        labels[left_indices] = decision_tree_predict(left_data, model['left_child'])
        labels[right_indices] = decision_tree_predict(right_data, model['right_child'])
