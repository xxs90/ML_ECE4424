...
def calculate_information_gain(data, labels):
    ...
    for c in range(num_classes):
        ...
        if np.any(nonzero_entries):
            with np.errstate(invalid='ignore', divide='ignore'):
                children_entropy = - np.multiply(np.multiply(prob_not_x, prob_y_given_not_x),
                                                 np.log(prob_y_given_not_x))
            gain[nonzero_entries] -= children_entropy[nonzero_entries]
...
def recursive_tree_train(data, labels, depth, max_depth, num_classes):
    ...
    # initialize a node
    class Node:
        def __init__(self, left_child=None, right_child=None, prediction=None, max_gain=None):
    ...
    node.max_gain = np.argmax(gain)

    # split the data
    split_indices = np.where(data[node.max_gain])
    split = np.ones(len(data.T), dtype=bool)
    split[split_indices, ] = False

    left_data = data.T[split_indices].T
    left_labels = labels[split_indices]
    right_data = data.T[split].T
...
def decision_tree_predict(data, model):
    ...
    for i, data in enumerate(data.T):
        labels[i] = predict_helper(data, model)

    return labels


def predict_helper(data, model):

    if model.prediction is not None:
        return model.prediction

    if data[model.max_gain]:
        return predict_helper(data, model.left_child)
    else:
        return predict_helper(data, model.right_child)
